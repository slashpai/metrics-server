...
var (
	...
	webconsoleBump = regexp.MustCompile(regexp.QuoteMeta("bump(github.com/kubernetes-incubator/metrics-server-web-console): ") + `([\w]+)`)
...
func main() {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		// has api changes
		display := fmt.Sprintf("%s [\\#%s](https://github.com/kubernetes-incubator/metrics-server/pull/%s)", message, matches[1], matches[1])
